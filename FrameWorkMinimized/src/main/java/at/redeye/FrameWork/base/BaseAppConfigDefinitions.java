...
{
    public static DBConfig DoLogging = new DBConfig("Log-Meldungen Schreiben", "NEIN", "Sollen Logmeldungen in einer LogDatei mitgeschrieben werden.", new PrmDefaultCheckSuite(PrmDefaultChecksInterface.PRM_IS_TRUE_FALSE));
    ...
    private static final DBConfig DateFormat = new DBConfig("DateFormat", "yyyy-MM-dd", "Datumsformat");
    public static DBConfig DisplayLanguage = new DBConfig("DisplayLanguage", "", "Sprache zb: en, oder de.");

    protected static void BaseRegisterDefinitions() {
        addLocal(DoLogging);
        addLocal(LoggingDir);
